...
class Mng_education extends CI_Controller
{
	...
	private function add_org_nodes_to_tree($category_tree, $visible_categories)
	{
		...
		foreach ($category_tree as $node) {
			...
			if (isset($node['data']) && $node['data']['type'] === 'category') {
				...
				// 조직 노드 추가
				foreach ($orgs as $org) {
					$edu_count = $this->Education_model->get_mng_org_edu_count($org['org_id']);
					$children[] = array(
						'key' => 'org_' . $org['org_id'],
				...
				$category_edu_count = $this->Education_model->get_mng_category_edu_count($category_idx);
				...
				$uncategorized_edu_count = $this->Education_model->get_mng_uncategorized_edu_count();
	...
	public function get_total_edu_count()
	{
		...
		$total_count = $this->Education_model->get_mng_total_edu_count($category_ids);
	...
	public function get_edu_list()
	{
		...
		if ($type === 'org' && $org_id) {
			...
			if (!empty($category_ids)) {
				$edu_list = $this->Education_model->get_mng_edu_list_by_categories($category_ids, $search_params);
			}
		} else {
			if (!empty($visible_categories)) {
				$category_ids = $this->Org_category_model->get_category_with_descendants_public($visible_categories);
				...
				$edu_list = $this->Education_model->get_mng_all_edu_list($search_params);
			}
		}
